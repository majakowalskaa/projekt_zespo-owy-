...
ggplot(lifestyle, aes(x = `Session_Duration (hours)`, 
                      y = Calories_Burned, 
                      color = Workout_Type)) +
  geom_point(alpha = 0.6, size = 2) +
  geom_smooth(method = "lm", se = FALSE) + # Linie trendu dla każdego typu
  labs(title = "Relacja: Czas trwania a spalone kalorie wg typu treningu",
...
plot_ly(df, x = ~diet_type, y = ~Fats,
        type = "box", color = ~diet_type)

plot_ly(df, x = ~diet_type, y = ~BMI,
        type = "box", color = ~diet_type)


ggplot(lifestyle, aes(x = diet_type, y = BMI, fill = diet_type)) +
  geom_violin(trim = FALSE, alpha = 0.6) + # Rysuje kształt rozkładu
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) + 
  labs(title = "Rozkład BMI w zależności od diety", 
       x = "Typ diety", 
       y = "BMI") +
  theme_minimal() +
...
lifestyle_long_bpm <- lifestyle %>%
  ...
  pivot_longer(cols = c(Resting_BPM, Avg_BPM), 
               names_to = "Typ_Tetna", 
               values_to = "BPM")
...
ggplot(lifestyle_long_bpm, aes(x = Experience_Level, y = BPM, fill = Typ_Tetna)) +
  ...
  scale_fill_manual(values = c("steelblue", "salmon"), 
                    labels = c("Średnie treningowe", "Spoczynkowe"))
...
ggplot(lifestyle, aes(x = `Workout_Frequency (days/week)`, y = Calories_Burned)) +
  ...
  facet_wrap(~Gender) + # Podział na panele Mężczyzna/Kobieta
