...
# Wykres interaktywny Plotly - Boxplot BMI
plot_ly(lifestyle, x = ~diet_type, y = ~BMI, type = "box", color = ~diet_type)
...
# Używamy koloru zależnego od metody gotowania, bo nie mamy zmiennej 'Zdrowy_posilek'
...
dane_do_korelacji <- lifestyle %>% 
  ...
  # Możesz też wybrać konkretne kolumny ręcznie, jeśli powyższe wybierze za dużo:
  # select(Age, `Weight (kg)`, `Height (m)`, BMI, Fat_Percentage, 
  #        `Session_Duration (hours)`, Calories_Burned, Avg_BPM, Resting_BPM, 
...
ggplot(dane_wykres, aes(x = Zmienna1, y = Zmienna2, fill = Korelacja)) + 
  ...
  labs(title = "Mapa Korelacji Zmiennych", x = "", y = "") + 
  coord_fixed()
```
